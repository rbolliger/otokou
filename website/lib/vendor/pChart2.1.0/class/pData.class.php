...
class pData {
    ...
    function getSerieAverage($Serie) {
        if (isset($this->Data["Series"][$Serie])) {
            ...
            $values = $this->stripVOID($this->Data["Series"][$Serie]["Data"]);
    ...
    function getSeriePercentile($Serie="Serie1", $Percentil=95) {
        ...
        $filtered_values = $this->stripVOID($this->Data["Series"][$Serie]["Data"]);
        if (!$filtered_values) {
            return NULL;
        }

        $Values = count($filtered_values) - 1;
        if ($Values < 0) {
            $Values = 0;
        }

        $PercentilID = floor(($Values / 100) * $Percentil + .5);
        $SortedValues = $filtered_values;
        sort($SortedValues);
